function JSONTable(tableObject)
{
	...
	{
		tableHeaderArray = []
		tableHeader = this.table.find("thead th")
		...
		for (var rc = 0; rc < tableRows.length; rc++)
		{
			...
			{
				tableJsonObject[tableHeaderArray[cc]] = $(tableRow[cc].firstElementChild).val()
			}
	...
	this.fromJSON = function(jsonSourceData = this.tableJSON, setFullJSON = true)
	{
		...
		for (var jr = 0; jr < jsonSourceData.length; jr++)
		{
			...
			{
				tableDataRow.append('<td><input type="text" value="'+jsonSourceData[jr][tableHeaderKeyArray[ki]] +'"</input></td>')
			}
